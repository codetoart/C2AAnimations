...
/**
 * Created by mobisys on 3/3/2017.
 */

public class C2AAnimationLoadingButton extends View {
    ...
    private int mRadius, mArcRadius;
    private int width;
    private int height;
    ...
    public C2AAnimationLoadingButton(Context context) {
        super(context);
        init(context, null);
    }

    public C2AAnimationLoadingButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public C2AAnimationLoadingButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.C2AAnimationLoadingButton, 0, 0);
            mColorPrimary = typedArray.getInt(R.styleable.C2AAnimationLoadingButton_c2a_primaryColor, Color.LTGRAY);
            mColorPrimaryDark = typedArray.getInt(R.styleable.C2AAnimationLoadingButton_c2a_primaryDarkColor, Color.LTGRAY);
            mLoadingColor = typedArray.getInt(R.styleable.C2AAnimationLoadingButton_c2a_loaderColor, Color.WHITE);
            String text = typedArray.getString(R.styleable.C2AAnimationLoadingButton_c2a_buttonText);
            mText = text == null ? "" : text;
            mTextColor = typedArray.getColor(R.styleable.C2AAnimationLoadingButton_c2a_textColor, Color.BLACK);
            mTextSize = typedArray.getDimensionPixelSize(R.styleable.C2AAnimationLoadingButton_c2a_textSize, 16);
            resetAfterFailed = typedArray.getBoolean(R.styleable.C2AAnimationLoadingButton_c2a_resetAfterFailed, true);
            mRippleAlpha = typedArray.getFloat(R.styleable.C2AAnimationLoadingButton_c2a_buttonRippleAlpha, 0.3f);
            typedArray.recycle();
        }
        ...
        ripplePaint.setColor(mColorPrimaryDark);
        ripplePaint.setAlpha((int) (mRippleAlpha * 255));
        ripplePaint.setStyle(Paint.Style.FILL);

        mStrokePaint = new Paint();
        mStrokePaint.setAntiAlias(true);
        mStrokePaint.setColor(mLoadingColor);
        ...
        mPathEffectPaint.setColor(mLoadingColor);
        ...
        mPathEffectPaint2.setColor(mLoadingColor);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mArcRadius = (int) (height - mArcPadding * 2) / 2;

        if (mButtonRectF == null) {
            mButtonRectF = new RectF();
            mButtonRectF.top = mPadding;
            mButtonRectF.bottom = height - mPadding;
            mArcRectF = new RectF(width / 2 - mArcRadius, mArcPadding, width / 2 + mArcRadius, height - mArcPadding);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (mCurrentState) {
            ...
            case STATE_ANIMATION_STEP1:
                ...
                if (mCurrentState == STATE_BUTTON) {
                    ...
                    if (mTouchX > 0 || mTouchY > 0) {
                        ...
                        canvas.drawRoundRect(mButtonRectF, cornerRadius, cornerRadius, ripplePaint);
                    }
                }
                break;
            ...
            case STATE_ANIMATION_LOADING:
                ...
                canvas.drawCircle(width / 2, height / 2, mRadius - mDensity, mPaint);
